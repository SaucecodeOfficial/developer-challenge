...
// quick exports
export { FeathersErrors } from './packages'
    ...
    return {
      models: t.notType(models, 'Function')
        ...
        : define => t.map(model => model(define))(
          models(createModel, Sequelize.DataTypes) || [],
        )
      ,
      ...
      configure: app => {
        ...
        if (t.not(t.eq(sequelize, null))) {
          ...
          app.setup = function (...args) {
            ...
            const syncOptions = t.eq(process.env.NODE_ENV, 'development')
              ? { alter: true }
              : { force: false }
    ...
    if (appFolderName) {
      if (t.isType(appFolderName, 'String')) {
        ...
        main.use((req, res, next) => {
          api.log('info', 'APP REQUEST -> ', req.originalUrl, namespace)
